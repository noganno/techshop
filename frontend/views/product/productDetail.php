...
if ($category != null) {
    ...
    $relatedProducts = $category->findActiveProducts()->andWhere(['!=', 'product.id', $model->id])->limit(10);
    $relatedProducts = \Yii::$app->db->cache(function ($db) use ($relatedProducts) {
        return $relatedProducts->all($db);
    });
...
$allAttributeGroups = $model->getAttributeGroups();
$allAttributeGroups = \Yii::$app->db->cache(function ($db) use ($allAttributeGroups) {
    return $allAttributeGroups->all($db);
});


$latestNews = \backend\modules\postmanager\models\News::find()->active()->latest(3);
$latestNews = \Yii::$app->db->cache(function ($db) use ($latestNews) {
    return $latestNews->all($db);
});
        ...
        <div class="product-page__info bestseller_new_recommended discount-yellow">
           ...
           <?= $model->getDiscountCircleLabel() ?>
			...
			c-0.535,0-0.971-0.437-0.971-0.972C9.439,26.256,9.875,25.821,10.41,25.821z"></path>
                ...
                <div class="links">
                    <?= a(t('Fast order'), ['fast-order/show', 'id' => $model->id], [
                        'class' => 'link-grey fast-order-modal-button',
                        'role' => 'modal-remote',
                    ]) ?>
        ...
        <div class="article">
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <div class="specification tab-pane fade show active" id="profile" role="tabpanel"
                    ...
                    <?php if (!empty($allAttributeGroups)): ?>
                        <?php foreach ($allAttributeGroups as $group): ?>
                            <?php
                            $characters = $group->getProductAttributes()
                                ->andWhere(['product_id' => $model->id]);

                            $characters = \Yii::$app->db->cache(function ($db) use ($characters) {
                                return $characters->all($db);
                            });
                ...
                <div class="carousel-cards owl-carousel owl-theme">
                    <?php foreach ($relatedProducts as $relatedProduct): ?>
                        ...
                        <div class="products-cards__item bestseller_new_recommended">
                            <a href="<?= url_to(['product/detail', 'id' => $relatedProduct->id]) ?>" class="img">
                                <img src="/thumb.php?src=<?= '/frontend/web' . $relatedProduct->getImage('card') ?>&w=140&h=140&a=t&zc=3">
                            </a>
                            <div class="content">
                                <span class="category"><?= $relatedProduct->getCategoryName() ?></span>
                                <a href="<?= url_to(['product/detail', 'id' => $relatedProduct->id]) ?>" class="name">
